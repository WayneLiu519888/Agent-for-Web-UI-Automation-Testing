...
**设计日期**: 2026-06-14 | **复杂度**: Large | **状态**: v0.1 精简版（8维->4扁维，三级字典->内联映射，Worker Pool 蓝图预留）
...
> **采集一切，Agent 按优先级消费。** Acc Tree v0.1 已从原始 8 维（dom/a11y/geometry/locators/interaction/framework/text/children）精简为 **4 扁维**（locators / a11y+text / geometry / interaction）。framework 和 text 合并入 dom 和 a11y；DomAttributes 从 24 字段精简到 8 字段；A11yInfo 移除 roledescription/readonly/multiline；GeometryInfo 移除 zIndex。Agent 执行用例时从定位器链中择优尝试，无需回查浏览器。
...
### 工具 3：`test-case-executor`（测试用例执行器 — 【蓝图预留，当前版本未实现】）
...
### 2.4 交互事件推断体系（v0.1 精简版 — 内联映射，三级字典已移除）

> **v0.1 精简版**：交互事件推断从三级字典（_overrides.yaml > components.yaml > base/controls.yaml，共624行）精简为内联 （约 16 个 role 映射，~50行）。不再依赖外部 YAML 字典文件。
...
### 2.5 交互事件推断器（InteractionInferrer — v0.1 内联映射版，~50行）
...
logging:
  ...
  max_size: "10MB"
  max_files: 5
n# HTTP 传输安全：IP 白名单。仅允许列表中指定的 IP 访问 /mcp 端点。
# 支持精确 IP（如 192.168.1.100）和 CIDR 子网（如 10.0.0.0/8）。
# 默认仅允许本机回环地址。添加远程客户端 IP 后重启服务生效。
ip_whitelist:
  - "127.0.0.1"
  - "::1"
```
...
```
Agent-for-Web-UI-Automation-Testing/  (v0.1 精简版)
|
|   ==========================================================
...
|
+-- docs/                     # 公开文档
|   +-- blueprint.plan.md     # 设计蓝图
|
+-- src/                      # 源码（框架引擎）
|   +-- index.ts              # 公共 API 导出
|   +-- types/                # 类型定义
|   |   +-- tool.ts           # MCP 工具定义接口
|   |   +-- yaml.ts           # AccTree(4扁维) / TestCase / Environment
|   |   +-- interaction-events.ts  # InteractionEvent 联合类型
|   |   +-- errors.ts         # 自定义错误
|   +-- server/
...
|   +-- capability/           # 能力层
|       +-- engine/           # 核心引擎（2 个文件）
|       |   +-- acc-tree.ts         # Acc Tree 4扁维采集 + domCollectScript
|       |   +-- interaction-inferrer.ts  # 内联 ROLE_EVENT_MAP（~50行）
|       +-- yaml/             # YAML IO (io.ts)
|       +-- config/           # 配置加载 (loader.ts)
|       +-- analysis/         # 组件分析 (2 个文件)
|       +-- tools/            # MCP 工具注册 (registry.ts)
|
+-- dictionaries/             # 交互事件字典（保留参考，v0.1 不再运行时加载）
|   +-- base/                 # 基础字典
|   +-- projects/             # 项目字典 — .gitignore
|
+-- enterprise/               # 企业机密层 — .gitignore 整目录禁止提交
    +-- configs/              # 企业覆盖配置
    +-- environments/         # 测试环境（URL/账号/租户）
    +-- test-cases/           # 测试用例 YAML
    +-- acc-trees/            # Acc Tree 快照
    +-- auth/                 # 登录态缓存
    +-- screenshots/          # 截图产出
    +-- reports/              # 测试报告
...
/test-case-executor — 【蓝图预留，当前版本未实现】
  ┌─────────────────────────────────────────────────────────────┐
  │ 【蓝图预留】Phase 1 — 批量推理 (主进程, LLM, 并发≤4)          │
  │   解析 YAML → Acc Tree(可选) → LLM → ExecutionPlan[]          │
  └──────────────┬──────────────────────────────────────────────┘
                 ▼
  ┌─────────────────────────────────────────────────────────────┐
  │ 【蓝图预留】Phase 2 — 并行执行 (Worker Pool)                  │
  │   当前版本未实现：Worker Pool / TaskScheduler / ResourceDetector│
  │   已从代码中删除                                              │
  └──────────────┬──────────────────────────────────────────────┘
                 ▼
  ┌─────────────────────────────────────────────────────────────┐
  │ 【蓝图预留】Phase 3 — 按需补救 (主进程, LLM)                  │
  └──────────────┬──────────────────────────────────────────────┘
                 ▼
  结果聚合 → JSON 报告 + 实时进度推送
...
| Phase | 内容 | 核心产出 | 状态 |
|-------|------|----------|:---:|
| 1 | 基础能力层 | AccTree 4扁维采集、Locator构建、YAML读写、InteractionInferrer(内联映射) | ✅ 完成 |
| 2 | YAML 类型体系 | types/yaml.ts + types/interaction-events.ts 精简版 | ✅ 完成 |
| 3 | 探索器 | explorer.ts + web-component-scout | 🔵 蓝图预留 |
| 4 | 执行器 | Worker Pool / TaskScheduler / ResourceDetector | 🔵 蓝图预留（已从代码删除） |
| 5 | init + 快照 + 用例生成 | web-init / web-snapshot / case-generator | 🔵 蓝图预留 |
| 6 | 集成 + 文档 | mcp.config.yaml, config loader, README | ⏳ 进行中 |
...
```
src/capability/    (v0.1 精简版)
├── engine/                           # 核心引擎（2 个文件）
│   ├── acc-tree.ts                   #   Acc Tree 4扁维采集 + 内联 domCollectScript
│   └── interaction-inferrer.ts       #   内联 ROLE_EVENT_MAP 事件推断（~50行）
├── yaml/                             # YAML IO
│   └── io.ts                         #   序列化写入（内联 FileWriteError）
├── config/                           # 配置加载
│   └── loader.ts                     #   双层配置加载 + 硬编码默认值
├── analysis/                         # 组件分析
│   ├── component-analyzer.ts         #   内联 KNOWN_ROLES 判定
│   └── component-scout.ts            #   交互式组件发现编排
└── tools/                            # MCP 工具注册
    └── registry.ts                   #   当前仅 web-snapshot
```
